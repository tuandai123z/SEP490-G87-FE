...
const CardItem = ({ item }) => {
  ...
  return (
    <div className="relative p-4 transition-all duration-300 ease-in-out bg-white border rounded-lg shadow-md w-3/10 group hover:shadow-lg hover:border-black">
      <div className="relative cursor-pointer" onClick={() => navigate(`/cua-hang/${item?.id}`)}>
        <img className="w-full h-40 bg-gray-300 " src={item?.url} />
      ...
      <div className="mt-3">
        <p className="overflow-hidden text-[0.75em] font-semibold text-gray-500 whitespace-nowrap uppercase">{item?.category}</p>
        <p className="text-[#334862] font-medium">{item?.name}</p>
        <p className="mt-1 font-bold text-[#334862]">
          Giá: <span className="text-red-500">{formatVND(item?.price)}</span>
        </p>
      </div>
    </div>
  );
